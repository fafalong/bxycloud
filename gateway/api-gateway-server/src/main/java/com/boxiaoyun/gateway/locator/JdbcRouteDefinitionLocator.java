...
public class JdbcRouteDefinitionLocator implements ApplicationListener<RemoteRefreshRouteEvent>, ApplicationEventPublisherAware {
    ...
    private final static String SELECT_LIMIT_PATH = "SELECT\n" +
            ...
            "    INNER JOIN system_api AS a ON i.api_id = a.api_id\n" +
     ...
     * 动态加载路由
     * * 示例
     * id: boxiaoyun-admin-provider
     * uri: lb://boxiaoyun-admin-provider
     * predicates:
     * - Path=/admin/**
     * - Name=平台后台管理服务
     * filters:
        ...
        try {
            ...
            if (limitApiList != null) {
                ...
                limitApiList.forEach(item -> {
                    ...
                    // 允许用户每秒处理多少个请求
                    long replenishRate = (item.getLimitQuota() / refreshInterval) < 1 ? 1 : refreshInterval;
                    // 令牌桶的容量
                    long burstCapacity = item.getLimitQuota();
                    ...
                    // 服务地址
                    URI uri = UriComponentsBuilder.fromUriString(StringUtil.isNotBlank(item.getUrl()) ? item.getUrl() : "lb://" + item.getServiceId()).build().toUri();
            ...
            if (routeList != null) {
                ...
                routeList.forEach(gatewayRoute -> {
                    ...
                    Map<String, String> predicatePathParams = new HashMap<>(8);
                    predicatePath.setName("Path");
                    predicatePathParams.put("name", StringUtil.isBlank(gatewayRoute.getRouteName()) ? gatewayRoute.getRouteId().toString() : gatewayRoute.getRouteName());
                    predicatePathParams.put("pattern", gatewayRoute.getPath());
                    predicatePathParams.put("pathPattern", gatewayRoute.getPath());
                    predicatePath.setArgs(predicatePathParams);
                    predicates.add(predicatePath);
                    // 服务地址
                    URI uri = UriComponentsBuilder.fromUriString(StringUtil.isNotBlank(gatewayRoute.getUrl()) ? gatewayRoute.getUrl() : "lb://" + gatewayRoute.getServiceId()).build().toUri();
